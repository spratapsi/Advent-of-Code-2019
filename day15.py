...
states = WALL, ALLOWED, OXYGEN = 0, 1, 2

instructions = {NORTH: 1j, SOUTH: -1j, WEST: -1, EAST: 1}

def find_oxy(position, state, dist=0, record=None):
    global computer
    record = record or {}
    ...
    for dir in directions:
        ...
        if new_state == WALL:
            ...
            return find_oxy(new_pos, new_state, new_dist, record)
            computer.input({1: 2, 2: 1, 3: 4, 4: 3}[dir])  # Go back
            computer.read()  # Throw away output


def find_all_positions(position, state, dist=0, record=None):
    global computer
    record = record or {}
    record[position] = (state, dist)
    for dir in directions:
        ...
        if new_state == WALL:
            ...
            find_all_positions(new_pos, new_state, new_dist, record)
...
record = find_all_positions(oxy_pos, state=OXYGEN)
